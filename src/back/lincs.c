...
lincs(x, xp, invmass, K, nrec, atom1, atom2, length, ncc, cmax, con, Sdiag, coef)
{
    ...
    for (i = 0; i < K; i++)  
    {
        float dx[i] = posi.x - posj.x;
        float dy[i] = posi.y - posj.y;
        float dz[i] = posi.z - posj.z;
        
        dx[i] -= fbox.x * rintf(dx[i]*fboxi.x);
        dy[i] -= fbox.y * rintf(dy[i]*fboxi.y);
        dz[i] -= fbox.z * rintf(dz[i]*fboxi.z);
    ...
    for (i = 0;  i < K; i++)
    {
        ...
        { 
            k = con[i, n]; 
            A[i, n] = coef[i, n]*(dx[i]*dx[j] + dy[i]*dy[j] + dz[i]*dz[j]);
        }
        a1 = atom1[i]; 
        a2 = atom2[i];
